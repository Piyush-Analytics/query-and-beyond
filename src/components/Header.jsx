...
const Header = ({ showLogoVideo }) => {
  ...
  useEffect(() => {
    ...
    const handleChange = (e) => { if (!localStorage.getItem("theme")) setDarkMode(e.matches); };
  ...
  return (
    <>
        ...
        >
          ...
          <div className="flex items-center gap-6">
            ...
            <a href="https://github.com/Piyush-Analytics" target="_blank" rel="noopener noreferrer">
              ...
              >
                <Github style={{ color: "var(--text-color)" }} className="h-5 w-5" />
              </button>
            </a>
            <button
              onClick={toggleTheme}
              ...
              className="p-2 rounded-full hover:opacity-80 transition-colors duration-300"
            >
              {darkMode ? <Sun style={{ color: "var(--text-color)" }} className="h-5 w-5" /> : <Moon style={{ color: "var(--text-color)" }} className="h-5 w-5" />}
            </button>
          </div>
        </div>
      ...
      {menuOpen && createPortal(
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-[999]" onClick={() => setMenuOpen(false)}>
          <div
          ...
          >
            <button className="absolute top-4 right-4 p-2 rounded-full bg-white/30 hover:bg-white/50 transition" onClick={() => setMenuOpen(false)}>
              <X className="h-6 w-6 text-black dark:text-white" />
            </button>
            <div className="mb-6">
              <h2 className="text-2xl font-bold text-white drop-shadow-lg">Navigation</h2>
              <p className="text-sm text-gray-200/80">Explore my work &amp; projects</p>
            </div>
            <ul className="flex flex-col gap-4 text-lg font-medium text-white">
              <Link to="/portfolio" onClick={() => setMenuOpen(false)} className="hover:scale-105 hover:text-blue-400 transition">Portfolio</Link>
              <Link to="/projects" onClick={() => setMenuOpen(false)} className="hover:scale-105 hover:text-blue-400 transition">Projects</Link>
            </ul>
            <div className="my-6 border-t border-white/30" />
            <div className="flex justify-center gap-4">
              <a href="https://github.com/Piyush-Analytics" target="_blank" rel="noopener noreferrer">
                <button style={{ backgroundColor: "var(--bg-color)", border: `1px solid var(--border-color)` }} className="p-3 rounded-full hover:scale-110 transition-transform">
                  <Github style={{ color: "var(--text-color)" }} className="h-5 w-5" />
                </button>
              </a>
              <button onClick={toggleTheme} style={{ backgroundColor: "var(--bg-color)", border: `1px solid var(--border-color)` }} className="p-3 rounded-full hover:scale-110 transition-transform">
                {darkMode ? <Sun style={{ color: "var(--text-color)" }} className="h-5 w-5" /> : <Moon style={{ color: "var(--text-color)" }} className="h-5 w-5" />}
              </button>
            </div>
          </div>
